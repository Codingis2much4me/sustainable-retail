...
def index():
    if request.method == 'POST':
        if 'run_strategic' in request.form:
            ...
            predictions = ai_core.run_strategic_prediction(PRODUCTS_LIST, target_month, target_year)
            ...
            try:
                ...
                return render_template('tactical_results.html', sale_items=sale_items.to_dict('records'), donation_table=donation_items.to_html(classes='table table-hover text-center', index=False), co2_saved=f"{co2_saved:.1f}", water_saved=f"{water_saved:,.0f}", revenue_recovered=f"${total_revenue_recovered:,.2f}", potential_meals=f"~{potential_meals:,}")
